...
describe('Workbench login and Dashboard',function()
{
    ...
    it.skip('Initial Reviewer Reopen',function(){
    ...
    it.skip('Curator Query Submit',function(){
        ...
        cy.get('.react-bootstrap-table').scrollTo('right')
        cy.get('.blue2').should('be.visible').click()
        cy.wait(10000)
        ...
        cy.contains('Ok').click()
        cy.on('window:alert', (str) => {
            expect(str).to.equal('The task will be submitted to the Reviewer')
        })
    })
    it.skip('Initial Reviewer Resolved',function(){
        cy.User2(CREDENTIALS.User2,CREDENTIALS.Pwd)
        // cy.contains('TestWF3').click()
        // cy.wait(2000)
        cy.get(':nth-child(3) > .nav-link').click({force:true})
    ...
    it.skip('Interim Reviewer Reopned Query',function(){
        cy.Lead(CREDENTIALS.Lead,CREDENTIALS.Pwd)
        cy.Lead(CREDENTIALS.Lead,CREDENTIALS.Pwd)
        Cypress.on('uncaught:exception', (err, runnable) => {  return false;  });
    ...
    it.skip('Curator Query subnit',function(){
        cy.User1(CREDENTIALS.User1,CREDENTIALS.Pwd)
        Cypress.on('uncaught:exception', (err, runnable) => {  return false;  });
        cy.contains('Automation Project').click().should('include.text','Automation Project')
        cy.get('#react-tabs-12').click().should('have.text','Query Tasks')
    ...
    it('Lead Resolve the query',function(){
        cy.Lead(CREDENTIALS.Lead,CREDENTIALS.Pwd)
        Cypress.on('uncaught:exception', (err, runnable) => {  return false;  });
        cy.contains('Automation Project').click().should('include.text','Automation Project')
        cy.get('#react-tabs-8').click().should('have.text','Interim Review')
